...
import { playwright, readFile } from "./release/index.js";
...
suite(
  ...
  (fixture) => {
    test("writes a viewport screenshot to a custom file", async () => {
      const path = `/tmp/viewport-${process.pid}.png`;
      const shot = await playwright.run(
        fixture.container,
      ...
      const file = await readFile(fixture.container, path);
    ...
    test("writes an element screenshot to a custom file", async () => {
